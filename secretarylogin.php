...
<form id="login" class="input-group" action="secretaryvalidation.php" method="post">
  ...
  <form id="register" class="input-group" action="<?php $PHP_SELF ;?>" method="post">
    <i class="fa fa-user" aria-hidden="true"></i>
...
if(isset($_POST["sub"])){
  if(!empty($_POST['name']) && !empty($_POST['pass']) && !empty($_POST['ID'])){
    ...
    if($numrows == 0){
        $sql="INSERT INTO secretary(USER_ID,PASSWORD,USERNAME) VALUES('".$ID."','".$pass."','".$name."')";
